...
interface IProps {
  open: boolean;
  initialValues?: ChurchLocationFormValues;
  mode: "create" | "edit";
...
const ChurchLocationModal = ({
  ...
  const { control, setValue, watch, handleSubmit, reset } =
    useForm<ChurchLocationFormValues>({
  ...
  const onSubmit = async (data: ChurchLocationFormValues) => {
    try {
  ...
  return (
    <Dialog open={open} onOpenChange={toggleOpen} modal>
      {/* FIX 10: Loading hanya tampil saat fetch province awal, bukan saat semua operasi */}
      {isLoadingProvince ? (
        <LoadingSection />
      ) : (
        <DialogContent className="sm:max-w-xl p-0 w-full h-[90vh] gap-0">
          <DialogHeader
            title="Location"
            description="Fill in the location details"
          />
          <form className="grid grid-cols-2 gap-6 p-4 overflow-y-auto">
            <Input
              label="Name"
...
export default ChurchLocationModal;
